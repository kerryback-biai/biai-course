...
| 22-30 | **Demo: Data Security in Action.** Ask the Meridian app for individual salary data — watch it return results. Then switch to a version with guardrails enabled: the same question is blocked or returns only aggregates. The data security problem made concrete. | Live demo |
